...
main()
{
  ...
  if $show_powerline; then
      ...
      # mpd
      tmux set-option -g status-right "#[fg=black,bg=blue]#(/home/arthur/.local/bin/tmux-mpd)"

      if $show_battery; then # battery
        tmux set-option -ag  status-right "#[fg=${pink},bg=${powerbg},nobold,nounderscore,noitalics] ${right_sep}#[fg=${dark_gray},bg=${pink}] #($current_dir/battery.sh)"
        powerbg=${pink}
      fi
  ...
  # Non Powerline Configuration
    ...
    #mpd
    tmux set-option -g status-right "#[fg=${white},bg=${dark_purple}]#($current_dir/mpd.sh)"
      ...
      # newsboat
      tmux set-option -ga status-right "#[fg=${dark_gray},bg=${cyan}] #($current_dir/rss.sh) "

      # redshift
      # tmux set-option -ga status-right "#[fg=${dark_gray},bg=${light_purple}] #($current_dir/redshift.sh) "
